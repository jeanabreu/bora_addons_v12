...
class MailMessage(models.Model):
    ...
    # -- Move messages
    def message_move(self, dest_model, dest_res_id, notify="0", lead_delete=False, opp_delete=False):
        """
        ...
        leads = False
        if lead_delete:
            lead_messages = self.env['mail.message'].search([('id', 'in', self.ids),
                                                             ('model', '=', 'crm.lead')])
        ...
        conversations = False
        conversation_messages = self.filtered(lambda m: m.model == 'cetmix.conversation')
        ...
        # Move messages
        if parent_message:
            self.sudo().write({'model': dest_model, 'res_id': dest_res_id, 'parent_id': parent_message.id})
        else:
            self.sudo().write({'model': dest_model, 'res_id': dest_res_id, 'parent_id': False})

        # Move attachments. Use sudo() to override access rules issues
        self.mapped('attachment_ids').sudo().write({'res_model': dest_model, 'res_id': dest_res_id})
        ...
        if dest_model == "cetmix.conversation":
            ...
            if conversation.message_ids:  # To ensure
                messages = conversation.message_ids.sorted(key=lambda m: m.id, reverse=True)
                conversation.update(
                    {
...
class MailMove(models.TransientModel):
    ...
    def message_move(self):
        ...
        # Move messages
        messages.message_move(self.model_to._name, self.model_to.id,
