...
class MailMessage(models.Model):
    ...
    @api.model
    def _unlink_trash_message(self):
    ...
    def _delete_conversations(self, conversation_ids):
        ...
        if len(conversations_2_archive) > 0:
            self.env["cetmix.conversation"].browse(conversations_2_archive).write(
                {"active": False}
            )
    ...
    def unlink_pro(self):
        ...
        if for_delete:
            count_delete = len(for_delete)
            for_delete.unlink()
            self._logging_message_deleted(count_delete)
    ...
    def _compute_author_display(self):
        for rec in self:
            rec.author_display = (
                ...
                else rec.email_from.replace(">", "").replace("<", "")
            )
