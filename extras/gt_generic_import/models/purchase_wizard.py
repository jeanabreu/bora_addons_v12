...
#           Search if not found it will create if create option is selected
            ...
            if product_id:
                ...
                if self.option=='create':
                    product_id=Product.create({'list_price':row[9],'barcode':str(int(row[5])) if isinstance(row[5],float) else row[5],'name':str(int(row[5])) if isinstance(row[5],float) else row[5],'type':'product'}).id
                else:
...
#           Search if not found it will create if create option is selected
            ...
            if uom_id:
                ...
                if self.option=='create':
                    ...
                    continue
#           Search if not found it will create if create option is selected
            cust_tax=Tax.search([('name','=',str(row[10])),('type_tax_use','=','purchase')],limit=1)
            if row[10]:
                if cust_tax:
                    cust_tax=cust_tax.id
                else:
                    if self.option=='create':
                        cust_tax=Tax.create({'name':str(row[10]),'type_tax_use':'purchase','amount':float(row[10])}).id
                    else:
