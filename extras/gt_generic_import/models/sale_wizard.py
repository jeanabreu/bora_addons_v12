...
class SaleOrder(models.TransientModel):
    ...
    state_stage = fields.Selection([('draft', 'Quotation'), ('purchase', 'Pedido confirmado')], string='Import State')

    #Secuencia de Sale Order
    @api.model
    def _get_number_sale_order(self):
    ...
    def Import_sale_order(self):
        ...
        for row in file_data:
           ...
           # if not row[0] or not row[1] or not row[2] or not row[4] or not row[5]:
            if not row[1] or not row[2] or not row[4] or not row[5]:
...
#           Search if not found it will create if create option is selected
            ...
            currency_id = Currency.search([('name', '=ilike', 'DOP')],limit=1)
            if pricelist_id:
...
#           Search if not found it will create if create option is selected
            ...
            purchase_vals.update({
                'name': name if self.seq_opt == 'f_sequence' else 'Nuevo',
                #'name': name if self.seq_opt == 'f_sequence' else self._get_number_sale_order(),
...
#           Search if not found it will create if create option is selected
            product_id=Product.search([('barcode','=',str(int(row[5])) if isinstance(row[5],float) else row[5]),('active','=',True)],limit=1)
            if product_id:
...
#           Search if not found it will create if create option is selected
            ...
            if uom_id:
                ...
                if self.option=='create':
                    ...
                    continue
#           Search if not found it will create if create option is selected
            #cust_tax=Tax.search([('name','=',float(row[8])),('type_tax_use','=','sale')],limit=1)
            cust_tax=Tax.search([('name','=',(row[8])),('type_tax_use','=','sale')],limit=1)
            if row[8]:
                if cust_tax:
                    cust_tax=cust_tax.id
                else:
                    if self.option=='create':
                        #cust_tax=Tax.create({'name':float(row[8]),'type_tax_use':'sale','amount':float(row[8])}).id
                        cust_tax=Tax.create({'name':(row[8]),'type_tax_use':'sale','amount':(row[8])}).id
                    else:
                        Log.create({'operation':'so','message':'Skipped could not find the tax with name %s'% float(row[8])})
                        continue
