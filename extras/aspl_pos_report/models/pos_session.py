...
class pos_session(models.Model):
    ...
    def get_gross_total(self):
        ...
        return round(gross_total, self.env['decimal.precision'].precision_get('Product Price'))
    ...
    def get_product_cate_total(self):
        ...
        return round(balance_end_real, self.env['decimal.precision'].precision_get('Product Price'))
                ...
                for line in order.lines:
                    ...
                    for lst in product_list:
                        if line.product_id.pos_categ_id:
                            if lst.get('pos_categ_id') == line.product_id.pos_categ_id.id:
                                lst['price'] = round(lst['price'] + (line.qty * line.price_unit) , self.env['decimal.precision'].precision_get('Product Price'))
                                flag = True
                        else:
                    ...
                    if not flag:
                        product_dict.update({
                                    ...
                                    'price': round((line.qty * line.price_unit), self.env['decimal.precision'].precision_get('Product Price')),
                                })
    ...
    def get_total_closing(self):
        if self:
            return round(self.cash_register_balance_end_real, self.env['decimal.precision'].precision_get('Product Price'))
    ...
    def get_total_sales(self):
        ...
        total_price = round(total_price, self.env['decimal.precision'].precision_get('Product Price'))
        return total_price
    ...
    def get_total_tax(self):
        ...
        total_tax = round(total_tax, self.env['decimal.precision'].precision_get('Product Price'))
        return total_tax
    ...
    def get_vat_tax(self):
        ...
        if self:
            ...
            for tax in self.env['account.tax'].browse(tax_list):
                ...
                taxes_info.append({
                    'tax_name': tax.name,
                    'tax_total': round(total_tax, decimals),
                    'tax_per': round(tax.amount,decimals),
                    'net_total': round(net_total, decimals),
                    'gross_tax': round(total_tax + net_total, decimals)
                })
        return taxes_info
    ...
    def get_total_discount(self):
        ...
        return round(total_discount, self.env['decimal.precision'].precision_get('Product Price'))
    ...
    def get_total_first(self):
        ...
        return round(total, self.env['decimal.precision'].precision_get('Product Price'))
...
# X - Report
    ...
    def get_total_sales_x(self):
        ...
        return round(total_price, self.env['decimal.precision'].precision_get('Product Price'))
