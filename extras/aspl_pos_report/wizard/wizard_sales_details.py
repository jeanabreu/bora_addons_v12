...
class wizard_sales_details(models.TransientModel):
    ...
    def get_total_sales(self, user_lst=None):
        if self:
            ...
            pos_ids = pos_obj.search([('date_order', '>=', str(self.start_date) + ' 00:00:00'), \
    ...
    def get_total_returns(self, user_lst=None):
        if self:
            ...
            company_id = user_obj.browse([self._uid]).company_id.id
            total_return = 0.0
            for pos in pos_order_obj.search([('date_order', '>=', str(self.start_date) + ' 00:00:00'),
                                             ('date_order', '<=', str(self.end_date) + ' 23:59:59'),
                                             ('user_id', 'in', user_ids), ('state', 'in', ['done', 'paid', 'invoiced']),
                                             ('company_id', '=', company_id)]):
    ...
    def get_tax_amount(self, user_lst=None):
        if self:
            ...
            pos_ids = pos_order_obj.search([('date_order','>=',str(self.start_date) + ' 00:00:00'),
                                            ('date_order','<=',str(self.end_date) + ' 23:59:59'),
