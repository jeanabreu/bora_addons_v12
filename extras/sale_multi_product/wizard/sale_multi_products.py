...
class sale_multi_products(models.TransientModel):
    ...
    product_ids = fields.Many2many('product.product', string='Multiples productos al presupuesto')
    do_replace = fields.Boolean('Reemplazar productos actuales por los seleccionados')

    def wizard_view(self):
        view = self.env.ref('sale_multi_product.view_sale_multi_products_wizard')

        return {
            'name': _('Seleccionar multiples productos'),
