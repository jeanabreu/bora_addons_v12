...
class PayrollReportView(models.Model):
    ...
    #MOD ext
    cotss = fields.Float(string='COTSS Salary')


    def _select(self):
        select_str = """
min(psl.id),ps.id,ps.number,emp.id as name,dp.id as department_id,jb.department_id as job_id,cmp.id as company_id,ps.date_from, ps.date_to, sum(psl.total) as net, sum(psl.code ='NET') as cotss, ps.state as state        """
